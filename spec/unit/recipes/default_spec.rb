...
describe_recipe 'git::default' do
  context %Q(with node['platform'] = 'ubuntu' AND node['platform_version'] = '14.04') do
    let(:chef_run) { ChefSpec::SoloRunner.new(platform: 'ubuntu', version: '14.04').converge(described_recipe) }
    it { expect(chef_run).to install_package('git') }
  end

  context %Q(with node['platform'] = 'ubuntu' AND node['platform_version'] = '12.04') do
    let(:chef_run) { ChefSpec::SoloRunner.new(platform: 'ubuntu', version: '12.04').converge(described_recipe) }
    it { expect(chef_run).to install_package('git') }
  end

  context %Q(with node['platform'] = 'ubuntu' AND node['platform_version'] = '10.04') do
  ...
  %w(redhat centos).each do |family|
    context %Q(with node['platform'] = '#{family}' AND node['platform_version'] = '5.10') do
  ...
  context %Q(with node['platform'] = 'windows' AND node['platform_version'] = '2012') do
  ...
  context %Q(with node['platform'] = 'omnios' AND node['platform_version'] = '151002') do
  ...
  context %Q(with node['platform'] = 'smartos' AND node['platform_version'] = '5.11') do
